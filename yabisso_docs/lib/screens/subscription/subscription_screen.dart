...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Widget _buildLogo() {
    return Container(
      ...
      child: const Icon(Icons.description, size: 40, color: AppColors.primaryTeal),
    );
  }
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Text('Connexion', textAlign: TextAlign.center,
                    style: TextStyle(fontWeight: FontWeight.bold, color: _isLogin ? AppColors.textDark : AppColors.grey)),
              ),
            ),
          ),
          Expanded(
  ...
  List<Widget> _buildRegisterFields() {
    return [
      ...
      const SizedBox(height: 20),
      _buildTextField(controller: _passwordController, label: 'Créer un mot de passe', hint: '••••••••', icon: Icons.lock_outline, obscure: _obscurePassword,
        suffixIcon: IconButton(icon: Icon(_obscurePassword ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: AppColors.grey), onPressed: () => setState(() => _obscurePassword = !_obscurePassword))),
      const SizedBox(height: 20),
      _buildTextField(controller: _confirmPasswordController, label: 'Confirmer le mot de passe', hint: '••••••••', icon: Icons.lock_outline, obscure: _obscureConfirm,
        suffixIcon: IconButton(icon: Icon(_obscureConfirm ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: AppColors.grey), onPressed: () => setState(() => _obscureConfirm = !_obscureConfirm))),
    ];
  }
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            _buildPlanTile(ctx, 'Micro', '10 documents max', Icons.description_outlined, const Color(0xFFF5A623)),
            _buildPlanTile(ctx, 'Basic', '25 documents max', Icons.description, AppColors.primaryBlue),
            _buildPlanTile(ctx, 'Premium', '50 documents max', Icons.diamond_outlined, AppColors.primaryTeal),
            _buildPlanTile(ctx, 'Illimité', 'Documents illimités', Icons.all_inclusive, const Color(0xFF7C3AED)),
            const SizedBox(height: 8),
  ...
  Future<void> _validateOfflineVoucher(String code) async {
    ...
    try {
      final boutiqueId = await OfflineVoucherService.getOrCreateBoutiqueId();
      if (boutiqueId == null) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Aucune boutique enregistrée."), backgroundColor: Colors.red));
        return;
      }
      ...
      if (limit != null) {
        await prefs.setInt('max_documents', limit);
      } else {
        await prefs.remove('max_documents');
      }
      await prefs.setBool('is_logged_in', true);
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2))),
            const SizedBox(height: 20),
            Container(width: 64, height: 64, decoration: const BoxDecoration(color: Color(0xFFFFF3E0), shape: BoxShape.circle),
                child: const Icon(Icons.monetization_on, color: AppColors.primaryAmber, size: 32)),
            const SizedBox(height: 16),
