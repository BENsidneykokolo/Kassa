...
class OfflineVoucherService {
  ...
  static String generateBoutiqueId(String phone, String name) {
    ...
    final last4 = phoneClean.length >= 4
  ...
  static Future<String?> getOrCreateBoutiqueId() async {
    ...
    phone = await secureStorage.read(key: 'user_phone');
  ...
  static Future<String?> validateOfflineVoucher(
      String code, String boutiqueId) async {
    final trimmedCode = code.trim().toUpperCase();
    final parts = trimmedCode.split('-');
    if (parts.length != 3 || parts[0] != 'OFF') {
      return 'Code invalide. Format attendu: OFF-XXXX-XXXX';
    }
    if (!_codeBelongsToBoutique(trimmedCode, boutiqueId)) {
      return 'Ce code ne vous appartient pas';
    }
    if (await isCodeUsed(trimmedCode)) {
      return "Ce code a déjà été utilisé. Vous avez besoin d'un nouveau voucher.";
    }
    return null;
  }
}
