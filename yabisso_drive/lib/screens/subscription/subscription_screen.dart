...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Widget _buildLogo() {
    return Container(
      ...
      child: const Icon(Icons.folder, size: 40, color: AppColors.primaryBlue),
    );
  }
        ...
        child: Column(
          children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(_error!, style: const TextStyle(color: AppColors.primaryRed, fontSize: 13)),
                    ),
                  ],
                ),
              ),
            ...
            SizedBox(
              ...
              height: 48,
              child: ElevatedButton.icon(
                onPressed: _showPointsPaymentDialog,
                icon: const Icon(Icons.monetization_on, size: 20),
                label: const Text('Payer avec points', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                style: ElevatedButton.styleFrom(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: _isLogin ? [BoxShadow(color: Colors.black.withValues(alpha: 0.08), blurRadius: 8)] : null,
                ),
                child: Text('Connexion', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, color: _isLogin ? AppColors.textDark : AppColors.grey)),
              ),
            ),
          ),
          Expanded(
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Text("S'inscrire", textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, color: !_isLogin ? AppColors.textDark : AppColors.grey)),
              ),
            ),
          ),
        ],
  ...
  List<Widget> _buildLoginFields() {
    return [
      _buildTextField(controller: _phoneController, label: 'Numéro de téléphone', hint: '+242 06 XXX XXXX', icon: Icons.phone_outlined, keyboardType: TextInputType.phone),
      const SizedBox(height: 20),
      _buildTextField(
        controller: _passwordController, label: 'Mot de passe', hint: '••••••••', icon: Icons.lock_outline, obscure: _obscurePassword,
  ...
  List<Widget> _buildRegisterFields() {
    return [
      _buildTextField(controller: _storeNameController, label: 'Nom du magasin / boutique', hint: 'Ex: Boutique Mama Jeanne', icon: Icons.store_outlined),
      const SizedBox(height: 20),
      _buildTextField(controller: _ownerNameController, label: 'Nom du propriétaire', hint: 'Ex: Jean-Pierre Mbala', icon: Icons.person_outline),
      const SizedBox(height: 20),
      _buildTextField(controller: _phoneController, label: 'Numéro de téléphone', hint: '+242 06 XXX XXXX', icon: Icons.phone_outlined, keyboardType: TextInputType.phone),
      const SizedBox(height: 20),
      _buildTextField(
        controller: _passwordController, label: 'Créer un mot de passe', hint: '••••••••', icon: Icons.lock_outline, obscure: _obscurePassword,
        suffixIcon: IconButton(
          icon: Icon(_obscurePassword ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: AppColors.grey),
          onPressed: () => setState(() => _obscurePassword = !_obscurePassword),
        ),
      ...
      const SizedBox(height: 20),
      _buildTextField(
        controller: _confirmPasswordController, label: 'Confirmer le mot de passe', hint: '••••••••', icon: Icons.lock_outline, obscure: _obscureConfirm,
        suffixIcon: IconButton(
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_isLogin) {
      ...
      if (savedPhone == null) {
        setState(() { _error = 'Aucun compte trouvé. Inscrivez-vous d\'abord.'; _loading = false; });
        return;
      }
      if (savedPass != null && phone == savedPhone && password == savedPass) {
        ...
        setState(() { _error = 'Numéro ou mot de passe incorrect'; _loading = false; });
      }
    } else {
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Abonnement', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: AppColors.primaryRed)),
                  const SizedBox(height: 4),
                  Text('Renouvellez pour continuer.', style: TextStyle(fontSize: 13, color: Colors.grey[700])),
                ],
              ),
            ),
            Container(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            Container(width: 64, height: 64, decoration: const BoxDecoration(color: Color(0xFFE3F2FD), shape: BoxShape.circle), child: const Icon(Icons.workspace_premium, size: 32, color: AppColors.primaryBlue)),
            const SizedBox(height: 16),
            const Text('Choisissez votre abonnement', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            Text('Sélectionnez le plan qui vous convient', style: TextStyle(fontSize: 14, color: Colors.grey[600])),
            const SizedBox(height: 20),
            _buildPlanTile(ctx, 'Micro', '10 fichiers max', Icons.store_mall_directory_outlined, const Color(0xFFF5A623)),
            _buildPlanTile(ctx, 'Basic', '25 fichiers max', Icons.storefront, AppColors.primaryBlue),
            _buildPlanTile(ctx, 'Premium', '50 fichiers max', Icons.diamond_outlined, AppColors.primaryGreen),
            _buildPlanTile(ctx, 'Illimité', 'Fichiers illimités', Icons.all_inclusive, const Color(0xFF7C3AED)),
            const SizedBox(height: 8),
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: OutlinedButton.icon(
          onPressed: () { Navigator.pop(ctx); _sendWhatsAppWithPlan(plan); },
  ...
  Future<void> _sendWhatsAppWithPlan(String plan) async {
    ...
    final message = Uri.encodeComponent('Bonjour, je souhaite $action Yabisso Drive $plan.$idText$prestataireText');
    if (mounted) {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('Les codes OFF-XXXX-XXXX ne nécessitent pas Internet', style: TextStyle(fontSize: 11, color: Colors.grey[600])),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (code.isEmpty) {
                ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Entrez un code'), backgroundColor: Colors.red));
                return;
              }
  ...
  Future<void> _validateVoucher(String code) async {
    ...
    try {
      ...
      await prefs.setString('subscription_expires', DateTime.now().add(const Duration(days: 30)).toIso8601String());
      await prefs.setString('subscription_start', DateTime.now().toIso8601String());
      await prefs.setString('subscription_plan', 'BASIC');
      await prefs.setBool('is_logged_in', true);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Abonnement activé !'), backgroundColor: Colors.green));
        context.go('/vendor-auth');
      }
    } catch (e) {
      if (mounted) {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2))),
            const SizedBox(height: 20),
            Container(width: 64, height: 64, decoration: const BoxDecoration(color: Color(0xFFFFF3E0), shape: BoxShape.circle), child: const Icon(Icons.monetization_on, color: AppColors.primaryAmber, size: 32)),
            const SizedBox(height: 16),
            const Text('Payer avec points', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            Text('Solde: $pointsBalance pts (${fmtPrice(pointsBalance.toDouble())})', style: TextStyle(fontSize: 14, color: Colors.grey[600])),
            const SizedBox(height: 20),
  ...
  Future<void> _activateWithPoints(String plan, int price) async {
    ...
    try {
      ...
      await prefs.setBool('is_logged_in', true);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Abonnement $plan activé ! $price points utilisés.'), backgroundColor: Colors.green));
        context.go('/vendor-auth');
      }
    } finally {
      if (mounted) setState(() => _loading = false);
  ...
  Future<void> _validateOfflineVoucher(String code) async {
    ...
    try {
      final boutiqueId = await OfflineVoucherService.getOrCreateBoutiqueId();
      if (boutiqueId == null) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Aucune boutique enregistrée. Inscrivez-vous d\'abord.'), backgroundColor: Colors.red));
        return;
      }
      final error = await OfflineVoucherService.validateOfflineVoucher(code, boutiqueId);
      if (error != null) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(error), backgroundColor: Colors.red, duration: const Duration(seconds: 4)));
        return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Abonnement activé avec succès !'), backgroundColor: Colors.green));
        context.go('/vendor-auth');
      }
    } finally {
      if (mounted) setState(() => _loading = false);
      ...
      child: RichText(
        ...
        text: TextSpan(
          ...
          children: [
            ...
            TextSpan(text: 'Conditions', style: TextStyle(color: Colors.grey[700], decoration: TextDecoration.underline, fontWeight: FontWeight.w500)),
            const TextSpan(text: ' et notre '),
            TextSpan(text: 'Politique de Confidentialité', style: TextStyle(color: Colors.grey[700], decoration: TextDecoration.underline, fontWeight: FontWeight.w500)),
          ],
        ),
      ),
    );
