...
class _VendorAuthScreenState extends ConsumerState<VendorAuthScreen> with SingleTickerProviderStateMixin {
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text('Vous devez souscrire à un abonnement pour accéder à Yabisso Drive.', textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: Colors.grey[600], height: 1.4)),
                  const SizedBox(height: 20),
                  _BoutiqueInfoWidget(),
                  const SizedBox(height: 20),
                  Container(
                    ...
                    child: Column(
                      children: [
                        Row(children: [
                          const Icon(Icons.vpn_key, size: 18, color: AppColors.primaryBlue),
                          ...
                          child: ElevatedButton(
                            onPressed: validating ? null : () async {
                              ...
                              if (code.startsWith('OFF-')) {
                                ...
                                if (limit != null) { await prefs.setInt('max_files', limit); } else { await prefs.remove('max_files'); }
                  ...
                  GestureDetector(
                    onLongPress: () {
                      ...
                      if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Numéro copié !'), duration: Duration(seconds: 1)));
                    },
                    child: SelectableText('242050332359', style: TextStyle(fontSize: 13, color: Colors.grey[500], decoration: TextDecoration.underline), textAlign: TextAlign.center),
                  ),
                ],
              ),
            ),
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            _buildPlanOption(ctx, 'Micro', '10 fichiers', Icons.store_mall_directory_outlined, const Color(0xFFF5A623)),
            _buildPlanOption(ctx, 'Basic', '25 fichiers', Icons.storefront, AppColors.primaryBlue),
            _buildPlanOption(ctx, 'Premium', '50 fichiers', Icons.diamond_outlined, AppColors.primaryGreen),
            _buildPlanOption(ctx, 'Illimité', 'Fichiers illimités', Icons.all_inclusive, const Color(0xFF7C3AED)),
            const SizedBox(height: 8),
  ...
  Widget _buildPlanOption(BuildContext ctx, String plan, String desc, IconData icon, Color color) {
    return Padding(
  ...
  Future<void> _openWhatsAppWithPlan(String plan) async {
    ...
    final message = Uri.encodeComponent('Bonjour, je souhaite prendre un abonnement Yabisso Drive $plan.$idText');
    if (mounted) {
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(width: 72, height: 72, decoration: const BoxDecoration(color: Color(0xFFE3F2FD), shape: BoxShape.circle), child: const Icon(Icons.vpn_key, size: 36, color: AppColors.primaryBlue)),
              const SizedBox(height: 20),
              const Text('Activer votre abonnement', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textDark)),
              const SizedBox(height: 8),
              Text('L\'administrateur vous a envoyé un code OFF. Collez-le ici.', textAlign: TextAlign.center, style: TextStyle(fontSize: 13, color: Colors.grey[600], height: 1.4)),
              const SizedBox(height: 20),
              TextField(
                controller: codeController,
                textCapitalization: TextCapitalization.characters,
                decoration: InputDecoration(hintText: 'OFF-XXXX-XXXX', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.vpn_key_outlined)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: AppColors.primaryBlue, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                  child: validating
                      ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
              ...
              const SizedBox(height: 12),
              TextButton(onPressed: () { Navigator.pop(ctx); if (mounted) _showSubscriptionRequiredDialog(); }, child: const Text('Retour', style: TextStyle(color: Colors.grey))),
            ],
          ),
        ),
      ),
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            TextButton(onPressed: () { Navigator.pop(ctx); WidgetsBinding.instance.addPostFrameCallback((_) => _showSubscriptionRequiredDialog()); }, child: const Text('Annuler', style: TextStyle(color: Colors.grey))),
          ],
        ),
      ),
    );
  ...
  Future<void> _activateWithPoints(String plan, int price) async {
    ...
    if (limit != null) await prefs.setInt('max_files', limit);
    if (limit == null) await prefs.remove('max_files');
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          const Text('Yabisso Drive', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18)),
        ],
      ),
    );
  }
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        const Text('Aucun utilisateur enregistré', style: TextStyle(fontSize: 16, color: Colors.grey)),
                        const SizedBox(height: 8),
                        const Text('Ajoutez un utilisateur pour commencer', style: TextStyle(fontSize: 13, color: Colors.grey)),
                        const SizedBox(height: 20),
                        SizedBox(
                        ...
                        const SizedBox(height: 12),
                        TextButton(onPressed: () => context.go('/'), child: const Text('Retour', style: TextStyle(color: Colors.grey))),
                      ],
                    ),
                  );
                }
  ...
  Widget _buildVendorCard(Vendor vendor) {
    final color = Color(int.parse(vendor.color?.replaceFirst('#', '0xFF') ?? '0xFF378ADD'));
        ...
        child: Column(
          ...
          children: [
            CircleAvatar(
              ...
              child: Text(vendor.initials ?? vendor.name.substring(0, 2).toUpperCase(), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: Colors.white)),
            ),
            const SizedBox(height: 12),
            Text(vendor.name, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF111111)), textAlign: TextAlign.center),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            CircleAvatar(radius: 36, backgroundColor: color, child: Text(vendor.initials ?? vendor.name.substring(0, 2).toUpperCase(), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: Colors.white))),
            ...
            if (_pinError) Padding(
              padding: const EdgeInsets.only(top: 8),
              child: Text(isFirstTime ? 'Les PIN ne correspondent pas' : 'PIN incorrect, réessayez', style: TextStyle(fontSize: 12, color: _coral)),
            ),
            const SizedBox(height: 32),
            _buildNumpad(),
            const SizedBox(height: 16),
            TextButton.icon(
              onPressed: () => setState(() { _showPinEntry = false; _selectedVendor = null; _pin = ''; _confirmPin = ''; _pinError = false; }),
    ...
    return Row(
      ...
      children: List.generate(4, (index) {
        ...
        return AnimatedContainer(
          ...
          child: Center(child: filled ? Container(width: 7, height: 7, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.white)) : null),
        );
      }),
    );
  }
  ...
  Widget _buildNumpad() {
    final keys = [['1', '2', '3'], ['4', '5', '6'], ['7', '8', '9'], ['cancel', '0', 'backspace']];
    return Column(
            ...
            children: row.map((key) {
              ...
              return Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    width: 64, height: 64,
                    decoration: BoxDecoration(shape: BoxShape.circle, color: (isCancel || isBackspace) ? const Color(0xFFFCF0F0) : const Color(0xFFF0EDEC)),
                    child: Center(
                      child: isCancel
                          ...
                          : isBackspace
                              ...
                              : Text(key, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Color(0xFF111111))),
                    ),
                  ),
                ),
              );
  ...
  void _selectVendor(Vendor vendor) {
    setState(() { _selectedVendor = vendor; _pin = ''; _confirmPin = ''; _showPinEntry = true; _pinError = false; _isFirstTime = vendor.pinHash == null; });
  }
  ...
  void _onKeyPress(String key) {
    setState(() {
      if (key == 'backspace') {
        ...
        _showPinEntry = false; _selectedVendor = null; _pin = ''; _confirmPin = ''; _pinError = false;
        return;
      } else {
  ...
  void _validatePin() async {
    ...
    if (isFirstTime) {
      if (_confirmPin.isEmpty) {
        ...
        if (_pin == _confirmPin) {
          await DatabaseHelper.instance.updateVendor(vendor.id!, {
            'pin_hash': BCrypt.hashpw(_pin, BCrypt.gensalt()),
          });
  ...
  void _showAddVendorDialog() {
    ...
    final roleController = TextEditingController(text: 'Utilisateur');
    final pinController = TextEditingController();
    final confirmPinController = TextEditingController();
    String selectedColor = '#378ADD';

    final colors = ['#378ADD', '#E24B4A', '#1D9E75', '#F5A623', '#9B59B6', '#1ABC9C', '#E67E22', '#34495E'];
        ...
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                TextField(
                  controller: nameController, autofocus: true, textCapitalization: TextCapitalization.words,
                  decoration: InputDecoration(hintText: 'Nom', border: const OutlineInputBorder(), focusedBorder: const OutlineInputBorder(borderSide: BorderSide(color: _primary))),
                ),
                const SizedBox(height: 12),
                TextField(
                  controller: roleController,
                  decoration: InputDecoration(hintText: 'Rôle (ex: Utilisateur, Admin)', border: const OutlineInputBorder(), focusedBorder: const OutlineInputBorder(borderSide: BorderSide(color: _primary))),
                ),
                const SizedBox(height: 12),
                TextField(
                  controller: pinController, keyboardType: TextInputType.number, maxLength: 4, obscureText: true,
                  decoration: const InputDecoration(hintText: 'PIN à 4 chiffres', counterText: '', border: OutlineInputBorder(), focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: _primary))),
                ),
                TextField(
                  controller: confirmPinController, keyboardType: TextInputType.number, maxLength: 4, obscureText: true,
                  decoration: const InputDecoration(hintText: 'Confirmer le PIN', counterText: '', border: OutlineInputBorder(), focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: _primary))),
                ),
                const SizedBox(height: 16),
                ...
                Wrap(
                  ...
                  children: colors.map((c) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        width: 36, height: 36,
                        decoration: BoxDecoration(color: color, shape: BoxShape.circle, border: Border.all(color: isSelected ? Colors.black : Colors.transparent, width: 3)),
            ...
            ElevatedButton(
              onPressed: () async {
                if (nameController.text.trim().isEmpty) {
                  if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Veuillez entrer un nom'), backgroundColor: Colors.orange));
                  return;
                }
                ...
                if (pin.isNotEmpty && pin.length != 4) {
                  if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Le PIN doit contenir 4 chiffres'), backgroundColor: Colors.orange));
                  return;
                }
                if (pin.isNotEmpty && pin != confirmPin) {
                  if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Les PIN ne correspondent pas'), backgroundColor: Colors.red));
                  return;
                }
                await DatabaseHelper.instance.insertVendor({
                  'name': name,
                  'role': roleController.text.trim().isNotEmpty ? roleController.text.trim() : 'Utilisateur',
                  'pin_hash': pin.isNotEmpty ? BCrypt.hashpw(pin, BCrypt.gensalt()) : null,
                  'color': selectedColor,
                  'initials': initials,
                });
                ref.invalidate(vendorsProvider);
                if (ctx.mounted) Navigator.pop(ctx);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$name ajouté avec succès'), backgroundColor: Colors.green));
                }
              },
    ...
    return FutureBuilder<Map<String, String?>>(
      ...
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFFF8F9FA), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE8E6E4))),
          child: Column(
            children: [
              if (boutiqueId != null && boutiqueId.isNotEmpty) ...[
                Row(children: [
                  const Icon(Icons.store, size: 14, color: AppColors.primaryBlue),
                  const SizedBox(width: 6),
                  const Text('ID Boutique: ', style: TextStyle(fontSize: 12, color: Color(0xFF999999))),
                  Expanded(child: SelectableText(boutiqueId, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF1565C0)))),
                ]),
                const SizedBox(height: 4),
              ],
              if (plan != null) ...[
                Row(children: [
                  ...
                  const SizedBox(width: 6),
                  Text('Plan: $plan', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF555555))),
                ]),
                const SizedBox(height: 4),
              ],
              if (expiresStr != null) ...[
                Row(children: [
                  ...
                  const SizedBox(width: 6),
                  Text('Expire: ${expiresStr.substring(0, 10)}', style: const TextStyle(fontSize: 12, color: Color(0xFF999999))),
                ]),
              ],
            ],
          ),
